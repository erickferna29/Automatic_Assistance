...
const SignalIcon = () => {
  // Animaciones para los 3 anillos de señal
  ...
  useEffect(() => {
    // Función que anima un solo anillo
  ...
  return (
    <View style={signalStyles.wrapper}>
      ...
      {/* Icono central */}
      <Animated.View
        style={[signalStyles.center, { transform: [{ scale: pulse }] }]}>
...
const WelcomeScreen = ({ navigation, route }) => {
  ...
  const primerNombre = (usuario?.nombre || 'Usuario')
    .split(' ')[0];
          ...
          <View style={styles.statusDots}>
            {statusMessages.map((_, i) => (
              <View
                ...
                style={[
                  styles.statusDot,
                  i <= step && styles.statusDotActive,
                  i === step && styles.statusDotCurrent,
                ]}
              />
            ))}
          </View>
