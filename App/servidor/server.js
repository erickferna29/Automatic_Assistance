/**
 * ============================================================
 *  SERVIDOR BACKEND ÔÇö Automatic Assistence
 *  Node.js + Express + MariaDB
 * ============================================================
 *
 *  INSTALACI├ôN (ejecuta en la carpeta /servidor):
 *    npm init -y
...
// ============================================================
//  CONFIGURACI├ôN DE LA BASE DE DATOS
//  ÔåÉ MODIFICA ESTOS VALORES SI CAMBIAN
// ============================================================
const DB_CONFIG = {
...
// Pool de conexiones (m├ís eficiente que una conexi├│n simple)
...
(async () => {
  try {
    ...
    console.log('Ô£à Conectado a MariaDB ÔÇö db_asistencia');
    conn.release();
  } catch (err) {
    console.error('ÔØî Error conectando a MariaDB:', err.message);
    console.error('   Verifica que XAMPP est├® corriendo y los datos de conexi├│n sean correctos.');
  }
})();

// ============================================================
//  CONFIGURACI├ôN DE MULTER (manejo de archivos/fotos)
//  Guarda la foto en memoria para luego insertarla como BLOB
// ============================================================
const storage = multer.memoryStorage(); // Foto en RAM ÔåÆ luego a LONGBLOB
const upload  = multer({
  storage,
  limits: { fileSize: 10 * 1024 * 1024 }, // M├íximo 10MB
  fileFilter: (req, file, cb) => {
    const allowed = ['image/jpeg', 'image/jpg', 'image/png'];
    if (allowed.includes(file.mimetype)) {
      cb(null, true);
    } else {
      cb(new Error('Solo se permiten im├ígenes JPG o PNG'));
    }
  },
});
  ...
  try {
    ...
    if (rows.length === 0) {
      return res.status(401).json({
        success: false,
        message: 'N├║mero de cuenta o NIP incorrecto.',
      });
    }
...
app.post('/auth/registro', async (req, res) => {
  ...
  if (nip.length < 4 || nip.length > 10) {
    return res.status(400).json({
      success: false,
      message: 'El NIP debe tener entre 4 y 10 d├¡gitos.',
    });
  }
  ...
  try {
    ...
    if (alumno.length === 0) {
      return res.status(404).json({
        success: false,
        message: 'N├║mero de cuenta no encontrado en el sistema.',
      });
    }
    ...
    return res.json({
      success: true,
      message: 'NIP registrado correctamente. Ahora puedes iniciar sesi├│n.',
    });
 ...
 *   - Campo "no_cuenta": n├║mero de cuenta del alumno
 *
...
app.post('/usuario/foto', upload.single('foto'), async (req, res) => {
  ...
  try {
    ...
    console.log(`[FOTO] Foto guardada para no_cuenta: ${no_cuenta} | Tama├▒o: ${(fotoBuffer.length / 1024).toFixed(1)} KB`);
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`\n­ƒÜÇ Servidor corriendo en http://localhost:${PORT}`);
