...
# Si una clase queda ACTIVA por más de este tiempo, el backend la finaliza solo.
# Esto evita que al reiniciar/prender el ESP32 se recupere una sesión vieja.
HORAS_MAXIMAS_SESION_ACTIVA = 4

# Ultimo resultado del reconocimiento facial para que la OLED pueda consultarlo.
# Se conserva en memoria porque solo interesa mostrar el evento mas reciente.
ultimo_evento_visual = {
    "evento_id": 0,
    "tipo": "",
    "no_cuenta": "",
    "mensaje": ""
...
def publicar_evento_visual(tipo: str, no_cuenta: str = ""):
    global ultimo_evento_visual
    nuevo_id = ultimo_evento_visual["evento_id"] + 1
    mensaje = f"Alumno ({no_cuenta}) encontrado" if tipo == "success" else "Rostro no reconocido"
    ultimo_evento_visual = {
        "evento_id": nuevo_id,
        "tipo": tipo,
...
def cerrar_sesiones_viejas(cursor):
    """
    Cierra automáticamente sesiones que quedaron ACTIVA por mucho tiempo.
    Esto evita que el ESP32 recupere una clase vieja al reiniciarse.
    """
    cursor.execute(
        f"""
        ...
        WHERE estado = 'ACTIVA'
          AND fecha_inicio < DATE_SUB(NOW(), INTERVAL {HORAS_MAXIMAS_SESION_ACTIVA} HOUR)
        """
    )


def obtener_sesion_activa_reciente(cursor):
    """
    Regresa únicamente una sesión ACTIVA reciente.
    Si una sesión activa es vieja, no se considera válida.
    """
    cursor.execute(
        f"""
        SELECT sc.id_sesion,
        ...
        WHERE sc.estado = 'ACTIVA'
          AND sc.fecha_inicio >= DATE_SUB(NOW(), INTERVAL {HORAS_MAXIMAS_SESION_ACTIVA} HOUR)
        LIMIT 1
        """
    )
...
def _calcular_asistencia_sesion(cursor, id_sesion: int) -> dict:
    ...
    for no_cuenta in todos_los_alumnos:
        ...
        cursor.execute(
            "SELECT id_asistencia, estado FROM Asistencia WHERE id_alumno = %s AND id_sesion = %s",
            (no_cuenta, id_sesion)
        )
        ...
        if existente:
            ...
            cursor.execute(
                "INSERT INTO Asistencia (id_alumno, id_sesion, estado) VALUES (%s, %s, %s)",
...
def iniciar_clase(clase: InicioClase):
    try:
        ...
        # 1. Validar Profesor
        cursor.execute(
            "SELECT no_empleado, nombre_profesor FROM Profesores WHERE no_empleado = %s",
            (clase.no_empleado,)
        )
        ...
        # 2. Validar Materia
        cursor.execute(
            "SELECT codigo_materia, nombre_materia FROM Materias WHERE codigo_materia = %s",
        ...
        cursor.execute(
            "SELECT 1 FROM Profesor_Materia WHERE id_profesor = %s AND id_materia = %s",
        ...
        cursor.execute(
            "INSERT INTO Sesiones_Clase (no_empleado, codigo_materia, estado) VALUES (%s, %s, 'ACTIVA')",
            (clase.no_empleado, clase.codigo_materia)
        )
        conexion.commit()
...
def finalizar_clase(data: FinClase):
    try:
        ...
        cursor.execute(
            "UPDATE Sesiones_Clase SET estado = 'FINALIZADA', fecha_fin = NOW() WHERE id_sesion = %s",
            (id_sesion,)
        )
...
def estado_clase():
    ...
    try:
        ...
        # Cierra sesiones viejas antes de responder al ESP32.
        cerrar_sesiones_viejas(cursor)
        conexion.commit()

        # Solo toma una sesión activa si todavía está dentro del tiempo permitido.
        sesion = obtener_sesion_activa_reciente(cursor)
        ...
        # Esta versión cuenta BLE + CAMARA.
        cursor.execute(
            "SELECT COUNT(DISTINCT no_cuenta) as total FROM Logs_Bluetooth WHERE id_sesion = %s",
            (sesion["id_sesion"],)
        )
...
def registrar_asistencia(registro: RegistroAsistencia):
    ...
    try:
        ...
        # Verificar sesión activa reciente
        sesion = obtener_sesion_activa_reciente(cursor)
        ...
        cursor.execute(
            "SELECT 1 FROM Alumno_Materia WHERE id_alumno = %s AND id_materia = %s",
        ...
        cursor.execute(
            "INSERT INTO Logs_Bluetooth (id_sesion, no_cuenta, fuente) VALUES (%s, %s, 'ESP32')",
            (id_sesion, no_cuenta)
        )
        conexion.commit()
        ...
        cursor.execute(
            "SELECT COUNT(*) AS total_logs FROM Logs_Bluetooth WHERE id_sesion = %s AND no_cuenta = %s",
            (id_sesion, no_cuenta)
        )
        ...
        cursor.execute(
            "SELECT COUNT(DISTINCT no_cuenta) AS detectados FROM Logs_Bluetooth WHERE id_sesion = %s",
            (id_sesion,)
        )
        detectados = cursor.fetchone()["detectados"]

        # Calcular estimado de la mínima en tiempo real para evitar NameError en el ESP32
        cursor.execute(
...
@app.get("/evento_visual")
def evento_visual():
    """Devuelve el ultimo resultado de la camara para mostrarlo en la OLED."""
    return ultimo_evento_visual
...
async def asistencia_visual(foto: UploadFile = File(...)):
    ...
    try:
        ...
        try:
            ...
            publicar_evento_visual("unknown")
        ...
        publicar_evento_visual("success", str(no_cuenta))
        ...
        cursor.execute(
            "SELECT id_asistencia FROM Asistencia WHERE id_alumno = %s AND id_sesion = %s",
            (no_cuenta, id_sesion)
        )
        existente = cursor.fetchone()
        ...
        if not existente:
            cursor.execute(
                "INSERT INTO Asistencia (id_alumno, id_sesion, estado) VALUES (%s, %s, 'PRESENTE')",
                (no_cuenta, id_sesion)
            )

            # Log simbólico separado de BLE para no confundirlo con advertisers.
            cursor.execute(
                "INSERT INTO Logs_Bluetooth (id_sesion, no_cuenta, fuente) VALUES (%s, %s, 'CAMARA')",
                (id_sesion, no_cuenta)
            )
            conexion.commit()
